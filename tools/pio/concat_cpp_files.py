...
using_plugin = ['P001','P003','P004','P005','P026','P029','P033','P034','P037']
...
def concat_P_cpp_files(path_to_concat):
    ...
    for root, dirs, files in os.walk(cpp_path):
        for file in files:
            if file.endswith('.cpp'):
                print("\u001b[33m Check: \u001b[0m  {} PlugIn if using".format(file[:4]))
                if file[:4] in using_plugin:
                    fullname = os.path.join(root, file)
                    cpp_files.append(fullname)
                    print("\u001b[33m Add: \u001b[0m  {}".format(fullname))
...
concat_P_cpp_files('./src/src/PluginStructs')
